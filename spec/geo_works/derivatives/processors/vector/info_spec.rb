...
RSpec.describe GeoWorks::Derivatives::Processors::Vector::Info do
  let(:processor) { described_class.new(path) }
  let(:path) { 'test.tif' }
  let(:polygon_info_doc) { read_test_data_fixture('files/ogrinfo_polygon.txt') }
  let(:line_info_doc) { read_test_data_fixture('files/ogrinfo_line.txt') }
